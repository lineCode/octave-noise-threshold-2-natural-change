...
{
	Array2D<Complexf> in_complex(in.w, in.h);
	forxy(in)
	{
		in_complex(p) = Complexf(in(p));
	}
	
	Array2D<Complexf> result(in.w, in.h);
...
inline Array2D<float> ifft(Array2D<Complexf> in, int flags)
{
	Array2D<Complexf> result(in.w, in.h);
	auto plan = fftwf_plan_dft_2d(in.h, in.w,
	...
	Array2D<float> out_real(in.w, in.h);
	forxy(in)
	{
